        ...
        const addFilter = (filter, options, isSpec) => {
          ...
          if (filter !== this.lastSelectedFilter) {
            ...
            if (filter === 'Categories') {
                const sizeSpec = ["Hair", "Beauty", "Zero Açúcar"]
                options = options.filter((option) => {
                    return sizeSpec.includes(option.key)
                })
            }
            if (filter === 'dias_de_tratamento') {
                const sizeSpec = ["60 dias", "120 dias", "180 dias", "240 dias", "300 dias", "360 dias"]
